...
function toggleMenuHidden()
{
	...
	$('#menuVel').toggleClass('hidden-phone', function()
	{
		if ($('.container:first').is('.menu-hidden'))
		{
		...
		if (typeof $.cookie != 'undefined')
			$.cookie('menuHidden', $('.container:first').is('.menu-hidden'));
	});
...
{
	if (!$('.container:first').is('.menu-hidden') && $('#menuVel').is('.hidden-phone'))
		$('#menuVel').removeClass('hidden-phone');
}
...
function genSparklines()
{
	if ($('.sparkline').length)
	{
		...
		$.each($('#menuVel .sparkline'), function(k)
		{
			var size = { w: 150, h: 20 };
...
var button_width = $('#supercheckout_confirm_order').width() + 76;
...
$(document).ready(function(){
    ...
    if(supercheckoutLayout == 3){
        ...
        var $wrapper = $('#customer_person_information_table');
...
$(function()
{
	...
	$('#menuVel .collapse').on('show.bs.collapse', function(e)
	{
		e.stopPropagation();
	...
	{
		/*
		if (!$('#menuVel').is(':visible') && !$('.container:first').is('.menu-hidden') && !$('.container:first').is('.documentation') && !$('.container:first').is('.login'))
		{
		...
		else
		{
			...
			if (!$('#menuVel').is(':visible') && parseInt($(document).width()) > 767 && !$('.container:first').is('.menu-hidden'))
			{
				$('.container:first').removeClass('menu-hidden');
				lastResizableMenuPosition();
			}
		}
		*/
...
//	$('[data-toggle="prettyPhoto"]').prettyPhoto();
	...
	{
		$('.container-960:first').toggleClass('menu-right');
		
		if ($(this).prop('checked')) 
			$('.container-960:first').removeClass('menu-left');
		else
			$('.container-960:first').addClass('menu-left');
	...
	if (typeof $.cookie != 'undefined' && $.cookie('rightMenu') && $('#toggle-menu-position').length)
	{
		...
		$('.container:first').not('.menu-right').removeClass('menu-left').addClass('menu-right');
	}
	...
	if (typeof $.cookie != 'undefined' && $.cookie('menuHidden') && $.cookie('menuHidden') == 'true' || (!$('.container').is('.menu-hidden') && !$('#menuVel').is(':visible')))
		toggleMenuHidden();
	else if ($('#menuVel').is(':visible'))
	{
		removeMenuHiddenPhone();
...
//	setTimeout(function()
//	{
//		var menu_max_height = parseInt($('#menuVel .slim-scroll').attr('data-scroll-height'));
//		var menu_real_max_height = parseInt($('#wrapper').height());
//		$('#menuVel .slim-scroll').slimScroll({
...
//		$('#menuVel .slim-scroll').trigger('mouseenter').trigger('mouseleave');
//	}, 200);
//	
//	/* Slim Scroll Widgets */
