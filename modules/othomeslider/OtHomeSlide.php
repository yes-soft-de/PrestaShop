...
class OtHomeSlide extends ObjectModel
{
	public $title;
	public $url;
	public $legend;
	...
	public static $definition = array(
		'table' => 'othomeslider_slides',
		'primary' => 'id_othomeslider_slides',
		'multilang' => true,
		'fields' => array(
	...
	public function add($autodate = true, $null_values = false)
	{
		...
		$res &= Db::getInstance()->execute('
			INSERT INTO `'._DB_PREFIX_.'othomeslider` (`id_shop`, `id_othomeslider_slides`)
	...
	public function delete()
	{
		...
		$res &= Db::getInstance()->execute('
			DELETE FROM `'._DB_PREFIX_.'othomeslider`
			WHERE `id_othomeslider_slides` = '.(int)$this->id
		);
	...
	public function reOrderPositions()
	{
		...
		$max = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			WHERE hss.`id_othomeslider_slides` = hs.`id_othomeslider_slides` AND hs.`id_shop` = '.(int)$id_shop
		);
		...
		$rows = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			SELECT hss.`position` as position, hss.`id_othomeslider_slides` as id_slide
			FROM `'._DB_PREFIX_.'othomeslider_slides` hss
			LEFT JOIN `'._DB_PREFIX_.'othomeslider` hs ON (hss.`id_othomeslider_slides` = hs.`id_othomeslider_slides`)
			WHERE hs.`id_shop` = '.(int)$id_shop.' AND hss.`position` > '.(int)$this->position
		);

		foreach ($rows as $row)
		{
			$current_slide = new OtHomeSlide($row['id_slide']);
