...
include_once(_PS_MODULE_DIR_.'othomeslider/OtHomeSlide.php');

class OtHomeSlider extends Module implements WidgetInterface
{
    protected $_html = '';
    ...
    public function __construct()
    {
        ...
        $this->displayName = $this->getTranslator()->trans('Ortho Themes Image slider', array(), 'Modules.Imageslider.Admin');
       
        $this->ps_versions_compliancy = array('min' => '1.7.1.1', 'max' => _PS_VERSION_);

        $this->templateFile = 'module:othomeslider/views/templates/hook/slider.tpl';
    }
    ...
    public function install()
    {
        ...
        if (parent::install() &&
            ...
            foreach ($shops as $shop_id) {
                ...
                $res = Configuration::updateValue('OTHOMESLIDER_SPEED', $this->default_speed, false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('OTHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover, false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('OTHOMESLIDER_WRAP', $this->default_wrap, false, $shop_group_id, $shop_id);
            }

            /* Sets up Shop Group configuration */
            if (count($shop_groups_list)) {
                foreach ($shop_groups_list as $shop_group_id) {
                    $res &= Configuration::updateValue('OTHOMESLIDER_SPEED', $this->default_speed, false, $shop_group_id);
                    $res &= Configuration::updateValue('OTHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover, false, $shop_group_id);
                    $res &= Configuration::updateValue('OTHOMESLIDER_WRAP', $this->default_wrap, false, $shop_group_id);
                }
            }

            /* Sets up Global configuration */
            $res &= Configuration::updateValue('OTHOMESLIDER_SPEED', $this->default_speed);
            $res &= Configuration::updateValue('OTHOMESLIDER_PAUSE_ON_HOVER', $this->default_pause_on_hover);
            $res &= Configuration::updateValue('OTHOMESLIDER_WRAP', $this->default_wrap);
    ...
    public function uninstall()
    {
        ...
        if (parent::uninstall()) {
            ...
            $res &= Configuration::deleteByName('OTHOMESLIDER_SPEED');
            $res &= Configuration::deleteByName('OTHOMESLIDER_PAUSE_ON_HOVER');
            $res &= Configuration::deleteByName('OTHOMESLIDER_WRAP');
    ...
    protected function createTables()
    {
        ...
        $res = (bool)Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'othomeslider` (
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'othomeslider_slides` (
              `id_othomeslider_slides` int(10) unsigned NOT NULL AUTO_INCREMENT,
        ...
        $res &= Db::getInstance()->execute('
            CREATE TABLE IF NOT EXISTS `'._DB_PREFIX_.'othomeslider_slides_lang` (
    ...
    protected function deleteTables()
    {
        ...
        foreach ($slides as $slide) {
            $to_del = new OtHomeSlide($slide['id_slide']);
            $to_del->delete();
        }

        return Db::getInstance()->execute('
            DROP TABLE IF EXISTS `'._DB_PREFIX_.'othomeslider`, `'._DB_PREFIX_.'othomeslider_slides`, `'._DB_PREFIX_.'othomeslider_slides_lang`;
        ');
    }
    ...
    public function getContent()
    {
        ...
        if (Tools::isSubmit('submitSlide') || Tools::isSubmit('delete_id_slide') ||
            ...
            if ($mode == 'add') {
                ...
                $associated_shop_ids = OtHomeSlide::getAssociatedIdsShop((int)Tools::getValue('id_slide'));
    ...
    protected function _postValidation()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            if (!Validate::isInt(Tools::getValue('OTHOMESLIDER_SPEED'))) {
    ...
    protected function _postProcess()
    {
        ...
        if (Tools::isSubmit('submitSlider')) {
            ...
            foreach ($shops as $shop_id) {
                ...
                $res = Configuration::updateValue('OTHOMESLIDER_SPEED', (int)Tools::getValue('OTHOMESLIDER_SPEED'), false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('OTHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('OTHOMESLIDER_PAUSE_ON_HOVER'), false, $shop_group_id, $shop_id);
                $res &= Configuration::updateValue('OTHOMESLIDER_WRAP', (int)Tools::getValue('OTHOMESLIDER_WRAP'), false, $shop_group_id, $shop_id);
            }

            /* Update global shop context if needed*/
            switch ($shop_context) {
                case Shop::CONTEXT_ALL:
                    $res &= Configuration::updateValue('OTHOMESLIDER_SPEED', (int)Tools::getValue('OTHOMESLIDER_SPEED'));
                    $res &= Configuration::updateValue('OTHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('OTHOMESLIDER_PAUSE_ON_HOVER'));
                    $res &= Configuration::updateValue('OTHOMESLIDER_WRAP', (int)Tools::getValue('OTHOMESLIDER_WRAP'));
                    if (count($shop_groups_list)) {
                        foreach ($shop_groups_list as $shop_group_id) {
                            $res &= Configuration::updateValue('OTHOMESLIDER_SPEED', (int)Tools::getValue('OTHOMESLIDER_SPEED'), false, $shop_group_id);
                            $res &= Configuration::updateValue('OTHOMESLIDER_PAUSE_ON_HOVER', (int)Tools::getValue('OTHOMESLIDER_PAUSE_ON_HOVER'), false, $shop_group_id);
                            $res &= Configuration::updateValue('OTHOMESLIDER_WRAP', (int)Tools::getValue('OTHOMESLIDER_WRAP'), false, $shop_group_id);
                        }
                    }
                    break;
            ...
            $slide = new OtHomeSlide((int)Tools::getValue('id_slide'));
            if ($slide->active == 0) {
                $slide->active = 1;
            } else {
                $slide->active = 0;
            ...
            $slide = new OtHomeSlide((int)Tools::getValue('delete_id_slide'));
            $res = $slide->delete();
            $this->clearCache();
            if (!$res) {
    ...
    {
    //    $this->context->controller->registerStylesheet('modules-othomeslider', 'modules/'.$this->name.'/css/othomeslider.css', ['media' => 'all', 'priority' => 150]);
    ...
    //    $this->context->controller->registerJavascript('modules-othomeslider', 'modules/'.$this->name.'/js/othomeslider.js', ['position' => 'bottom', 'priority' => 150]);
    }
    ...
    public function getWidgetVariables($hookName = null, array $configuration = [])
    {
        ...
        return [
            'othomeslider' => [
                'speed' => $config['OTHOMESLIDER_SPEED'],
                'pause' => $config['OTHOMESLIDER_PAUSE_ON_HOVER'] ? 'hover' : '',
                'wrap' => $config['OTHOMESLIDER_WRAP'] ? 'true' : 'false',
                'slides' => $slides,
            ],
        ];
    }
    ...
    public function hookActionShopDataDuplication($params)
    {
        Db::getInstance()->execute('
            INSERT IGNORE INTO '._DB_PREFIX_.'othomeslider (id_othomeslider_slides, id_shop)
            SELECT id_othomeslider_slides, '.(int)$params['new_id_shop'].'
            FROM '._DB_PREFIX_.'othomeslider
    ...
    public function getNextPosition()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
            ...
            FROM `'._DB_PREFIX_.'othomeslider_slides` hss, `'._DB_PREFIX_.'othomeslider` hs
            WHERE hss.`id_othomeslider_slides` = hs.`id_othomeslider_slides` AND hs.`id_shop` = '.(int)$this->context->shop->id
        );
    ...
    public function getSlides($active = null)
    {
        ...
        $slides = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT hs.`id_othomeslider_slides` as id_slide, hss.`position`, hss.`active`, hssl.`title`,
            hssl.`url`, hssl.`legend`, hssl.`image`
            FROM '._DB_PREFIX_.'othomeslider hs
            LEFT JOIN '._DB_PREFIX_.'othomeslider_slides hss ON (hs.id_othomeslider_slides = hss.id_othomeslider_slides)
            LEFT JOIN '._DB_PREFIX_.'othomeslider_slides_lang hssl ON (hss.id_othomeslider_slides = hssl.id_othomeslider_slides)
        ...
        foreach ($slides as &$slide) {
            $slide['image_url'] = $this->context->link->getMediaLink(_MODULE_DIR_.'othomeslider/images/'.$slide['image']);
        }
    ...
    public function getAllImagesBySlidesId($id_slides, $active = null, $id_shop = null)
    {
        ...
        $results = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'othomeslider_slides hss ON (hs.id_othomeslider_slides = hss.id_othomeslider_slides)
            LEFT JOIN '._DB_PREFIX_.'othomeslider_slides_lang hssl ON (hss.id_othomeslider_slides = hssl.id_othomeslider_slides)
            WHERE hs.`id_othomeslider_slides` = '.(int)$id_slides.' AND hs.`id_shop` = '.(int)$id_shop.
            ($active ? ' AND hss.`active` = 1' : ' ')
        );
    ...
    {
        $req = 'SELECT hs.`id_othomeslider_slides` as id_slide
                FROM `'._DB_PREFIX_.'othomeslider` hs
                WHERE hs.`id_othomeslider_slides` = '.(int)$id_slide;
    ...
    public function renderList()
    {
        ...
        foreach ($slides as $key => $slide) {
            ...
            $associated_shop_ids = OtHomeSlide::getAssociatedIdsShop((int)$slide['id_slide']);
            ...
            'form' => array(
                ...
                'input' => array(
                    ...
                    array(
                        ...
                        'name' => 'OTHOMESLIDER_PAUSE_ON_HOVER',
    ...
    public function getConfigFieldsValues()
    {
        ...
        return array(
            'OTHOMESLIDER_SPEED' => Tools::getValue('OTHOMESLIDER_SPEED', Configuration::get('OTHOMESLIDER_SPEED', null, $id_shop_group, $id_shop)),
            'OTHOMESLIDER_PAUSE_ON_HOVER' => Tools::getValue('OTHOMESLIDER_PAUSE_ON_HOVER', Configuration::get('OTHOMESLIDER_PAUSE_ON_HOVER', null, $id_shop_group, $id_shop)),
            'OTHOMESLIDER_WRAP' => Tools::getValue('OTHOMESLIDER_WRAP', Configuration::get('OTHOMESLIDER_WRAP', null, $id_shop_group, $id_shop)),
        );
    }
    ...
    protected function getShopContextError($shop_contextualized_name, $mode)
    {
        ...
        if ($mode == 'edit') {
            ...
            sprintf($this->getTranslator()->trans('You can only edit this slide from the shop(s) context: %s', array(), 'Modules.Imageslider.Admin'), $shop_contextualized_name) .
            '</p>';
        } else {
            return '<p class="alert alert-danger">' .
            sprintf($this->getTranslator()->trans('You cannot add slides from a "All Shops" or a "Group Shop" context', array(), 'Modules.Imageslider.Admin')) .
            '</p>';
        }
    }
    ...
    {
        return '<p class="alert alert-danger">'.
                        sprintf($this->getTranslator()->trans('Unable to get slide shop association information (id_slide: %d)', array(), 'Modules.Imageslider.Admin'), (int)$id_slide).
                '</p>';
    }
    ...
    protected function getCurrentShopInfoMsg()
    {
        ...
        if (Shop::isFeatureActive()) {
            if (Shop::getContext() == Shop::CONTEXT_SHOP) {
                $shop_info = sprintf($this->getTranslator()->trans('The modifications will be applied to shop: %s', array(),'Modules.Imageslider.Admin'), $this->context->shop->name);
            } else if (Shop::getContext() == Shop::CONTEXT_GROUP) {
                $shop_info = sprintf($this->getTranslator()->trans('The modifications will be applied to this group: %s', array(), 'Modules.Imageslider.Admin'), Shop::getContextShopGroup()->name);
            } else {
                $shop_info = $this->getTranslator()->trans('The modifications will be applied to all shops and shop groups', array(), 'Modules.Imageslider.Admin');
            }
    ...
    {
        return '<p class="alert alert-warning">'.
                    $this->getTranslator()->trans('This slide is shared with other shops! All shops associated to this slide will apply modifications made here', array(), 'Modules.Imageslider.Admin').
                '</p>';
    }
}
