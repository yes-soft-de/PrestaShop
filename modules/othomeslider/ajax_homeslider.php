...
include_once('othomeslider.php');

$home_slider = new OtHomeSlider();
$slides = array();
...
if (Tools::getValue('action') == 'updateSlidesPosition' && Tools::getValue('slides'))
{
	...
	foreach ($slides as $position => $id_slide)
		$res = Db::getInstance()->execute('
			UPDATE `'._DB_PREFIX_.'othomeslider_slides` SET `position` = '.(int)$position.'
			WHERE `id_othomeslider_slides` = '.(int)$id_slide
		);
